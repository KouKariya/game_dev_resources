		...
		while i < len(collidables):
			...
			if anObject != otherObject:
				...
				if distance < otherObject["radius"] + anObject["radius"]:
					...
					theOtherObjectDirection = math.atan2(otherObject["velocity"][1], anObject["velocity"][0])
					#Now calculate new X/Y values of each object for collision
					anObjectsNewVelocityX = anObjectSpeed * math.cos(anObjectDirection - collisionAngle)
					anObjectsNewVelocityY = theOtherObjectSpeed * math.sin(theOtherObjectDirection - collisionAngle)
...
def handleMouseDown():
	...
	currentObject = {"radius" : 3, "mass" : 3, "velocity" : [0,0], "position" : [0,0]}
