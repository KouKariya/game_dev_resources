...
while True:
	...
	#draw the planets in their new position
	drawUI()
	calcualteMovement()
	drawPlanets()

	#If one of the users has created a new planet,
	#draw it where the mouse is
	if currentBody is not None:
		drawCurrentBody()

		#If our user has released the mouse, add the new
	...
	#Draw the logo for the first four seconds
	if GAME_TIME.get_ticks() < 4000:
		surface.blit(logo, (108,77))

	#Store the previous mouse coordinates to create,
	#a vector when we release a new planet
